...
import { Drawer, Tag, Divider, Button, Typography, Space } from "antd";
    ...
    >
      ...
      <div className="p-6 flex flex-col gap-8">
              ...
              <div key={idx} className="flex justify-between items-center bg-white p-3 rounded-2xl border border-gray-100 hover:border-indigo-200 transition-colors">
                <div className="flex gap-4 items-center">
                  <div className="w-14 h-14 bg-gray-50 rounded-xl border border-gray-100 overflow-hidden shrink-0">
                    <img 
                      src={item.cake_id?.image_url || item.cake_id?.imageUrl} 
                      alt={item.cake_id?.name} 
                      className="w-full h-full object-cover" 
                    />
                  </div>
                  <div>
                    <div className="font-bold text-gray-900 text-sm leading-tight mb-1">{item.cake_id?.name}</div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400 font-bold font-mono">x{item.quantity}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right flex flex-col items-end gap-1.5">
                  <div className="font-black text-gray-800 text-sm">{formatPrice(item.price_at_buy * item.quantity)}</div>
                  {order.status === 'DONE' && (
                    <Button 
                      type="text" 
                      size="small" 
                      icon={<StarOutlined className="text-amber-500" />}
                      onClick={() => onReview(item?.cake_id?._id || item?.cake_id, item?.cake_id?.name)}
          ...
          <div className="space-y-4">
            ...
            {order.coupon_code && (
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-bold text-green-300">-{formatPrice(order.discount_amount || 0)}</span>
              </div>
            )}
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
           ...
           <Button type="primary" className="h-12 rounded-2xl font-black uppercase text-[11px] tracking-widest bg-indigo-600 shadow-lg shadow-indigo-200">Mua lại ngay</Button>
        </div>
      </div>
    </Drawer>
  );
